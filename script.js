...
// --- STAV HRY ---
let currentUser = null; // Vždy začínáme jako null pro vynucení loginu
...
    if(!currentUser) document.getElementById('login-modal').style.display = 'block'; 
}
...
function setHP() { 
    mHP = Math.round(10 * Math.pow(1.3, stage)) * (stage % 10 === 0 ? 5 : 1); 
    mCurr = mHP; 
}
...
window.unequip = function(t) { 
    if(!eq[t] || inv.length >= 8) return; 
    inv.push(eq[t]); delete eq[t]; renderChar(); updateUI(); save(); 
}
...
window.onload = async () => {
    // Vynucení loginu: Smazání jména a zobrazení modálu
    localStorage.removeItem('tt_user_v6'); 
    currentUser = null;
    
    document.getElementById('login-modal').style.display = 'block';
    document.getElementById('user-display').innerText = "Hráč";

    // Příprava základního UI před loginen
    if (raw.stage) {
        setHP();
    }
    updateBiome(); 
